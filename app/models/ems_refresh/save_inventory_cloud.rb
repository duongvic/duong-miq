module EmsRefresh::SaveInventoryCloud
  ...
  def save_backup_schedules_inventory(ems, hashes, target = nil)
    target = ems if target.nil?

    ems.backup_schedules.reset
    deletes = determine_deletes_using_association(ems, target)
    ...
    save_inventory_multi(ems.backup_schedules, hashes, deletes, [:ems_ref], nil, [:tenant, :availability_zone, :base_snapshot])
    store_ids_for_new_records(ems.backup_schedules, hashes, :ems_ref)
  end
