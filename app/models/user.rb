$LOAD_PATH.unshift(File.join(Rails.root, 'app/grpc/user')) unless $LOAD_PATH.include?(File.join(Rails.root, 'app/grpc/user'))
require 'grpc'
...
class User < ApplicationRecord
  ...
  before_create :pre_create_user

  before_destroy :check_reference, :prepend => true

  has_many :user_profiles, :foreign_key => :evm_owner_id
  ...
  validates :email, :format => {:with => MoreCoreExtensions::StringFormats::RE_EMAIL,
                                :allow_nil => true, :message => "must be a valid email address"}, :unique_within_region => {:match_case => false}
  ...
  def new_token
    ...
    JWT.encode(payload, Rails.application.config.default_key)
  end

  def new_device_token(id, ip)
    payload = {
      :device_id => id,
      :ip_address => ip,
    }
  ...
  def create_backup_service_user
    backup_stub = GrpcUser::UserService::Stub.new(Rails.application.config.backup_grpc, :this_channel_is_insecure)
    backup_user = GrpcUser::User.new(
      :id => self.id,
      :userid => self.userid,
      :name => self.name,
      :email => self.email,
      :phone_number => self.phone_number,
      :role => self.miq_user_role.name,
      :status => self.status,
      :password_digest => self.password_digest,
      :enable_two_factors => self.enable_two_factors,
      )
    backup_stub.create_user(backup_user)
  rescue GRPC::BadStatus => e
    _log.error "Error creating backup user: #{e.code} - #{e.details}"
    puts "Error creating backup user: #{e.code} - #{e.details}"
  end
  ...
  def pre_create_user
    ...
    self.password_expiration = Time.now.utc + 100.days
  end
