...
class OperatingSystem < ApplicationRecord
  ...
  def self.normalize_os_name(os_name)
    osName = os_name || 'unknown-NA'
    os_name_arr = osName.split("-")
    guest_os = "#{os_name_arr[0].to_s.downcase}-#{os_name_arr[1]}".capitalize
    guest_os
  end
  ...
  def self.simplify_name(os_name)
    findStr = os_name.downcase.gsub(/[^a-z0-9]/, "")
    @@os_map.each do |a|
      a[1].each do |n|
  ...
  def self.simple_image_name(obj)
    osName = nil
    ...
    if os
      osName = [:distribution, :product_type, :product_name].each do |field|
        os_field = os.send(field)
        break(os_field) if os_field && OperatingSystem.simplify_name(os_field) != "unknown"
      end

      if osName.kind_of?(String)
        osName = nil if OperatingSystem.simplify_name(osName) == "unknown"
      else
        osName = nil
      end
    end
    ...
    if osName.nil? && obj.hardware && !obj.hardware.guest_os.nil?
      osName = obj.hardware.guest_os
      norm_os = OperatingSystem.simplify_name(osName)
      if norm_os == "linux_generic" || norm_os == "unknown"
        vm_name = OperatingSystem.simplify_name(obj.name)
        return vm_name unless vm_name == "unknown"
      end
    end
    osName = obj.name if osName.nil?
    OperatingSystem.simplify_name(osName)
  end

  def self.platform(obj)
    simple_image_name(obj).split("_").first.capitalize
  end
end
