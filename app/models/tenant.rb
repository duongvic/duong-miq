...
class Tenant < ApplicationRecord
  ...
  after_create :create_default_tenant_quotas

  before_destroy :ensure_can_be_destroyed
  before_destroy :destroy_all_quotas
  ...
  has_many :cloud_networks, :inverse_of => :tenant
  has_many :cloud_subnets, :inverse_of => :tenant
  has_many :load_balancers, :inverse_of => :tenant
  has_many :security_groups, :inverse_of => :tenant
  has_many :firewall_rules, :inverse_of => :tenant
  has_many :network_routers, :inverse_of => :tenant
  has_many :cloud_volumes, :inverse_of => :tenant
  has_many :cloud_volume_backups, :inverse_of => :tenant
  has_many :backup_schedules, :inverse_of => :tenant
  has_many :cloud_volume_snapshots, :inverse_of => :tenant
  ...
  has_many :shares
  has_many :authentications, :dependent => :nullify, :inverse_of => :tenant
  ...
  def get_quota(quota_name)
    tenant_quotas.select {|quota| quota.name == "#{quota_name}_allocated"}.first
  end
  ...
  def create_default_tenant_quotas
    TenantQuota.create_default_quotas(self)
  end

  def destroy_all_quotas
    tenant_quotas.delete_all
  end
end
