module VmOrTemplate::Operations::Configuration
  ...
  def change_admin_password(server_id, options)
    raise _("VM has no EMS, unable to change Admin Password") unless ext_management_system

    raw_change_admin_password(server_id, options)
  end

  def raw_change_admin_password(_server_id, _options)
    raise NotImplementedError, _("must be implemented in a subclass")
  end
end
