...
class VmOrTemplate < ApplicationRecord
  ...
  virtual_column :os_simple_image_name,                        :type => :string,     :uses => [:operating_system, :hardware]
  ...
  def os_simple_image_name
    name = OperatingSystem.simple_image_name(self)
    if name == 'unknown'
      parent = genealogy_parent
      name = OperatingSystem.simple_image_name(parent) unless parent.nil?
    end
    name
  end
