class MiqGroup < ApplicationRecord
  ...
  def self.seed
    ...
    roles  = MiqUserRole.where("name like '%'").index_by(&:name)
    ...
    role_map.each_with_index do |(group_name, role_name), index|
      ...
      if user_role.nil?
        raise StandardError,
              _("Unable to find user_role '%{role_name}' for group '%{group_name}'") %
  ...
  def self.create_default_tenant_group(user)
    tenant_parent = Tenant.find(Rails.application.config.default_miq_tenant)
    tenant = Tenant.create(:name => user.userid, :description => "#{user.userid}",
                           :divisible => false, :parent => tenant_parent)

    if tenant.invalid?
      user.update_attribute(:status, false)
      return false, "Internal Server Error, something went wrong!"
    end

    miq_group = MiqGroup.create(:description => "#{user.userid}",
                                :miq_user_role => MiqUserRole.find(Rails.application.config.default_miq_role),
                                :tenant => tenant)
    if miq_group.invalid?
      user.update_attribute(:status, false)
      TenantQuota.where(:tenant_id => tenant.id).delete_all
      tenant.delete
      return false, "Internal Server Error, something went wrong!"
    end

    user.update!(:miq_groups => [miq_group])
    tenant.update_attribute(:default_miq_group, miq_group)
    user.update_attribute(:current_group, miq_group)
    return true, nil
  end
