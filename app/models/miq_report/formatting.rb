...
module MiqReport::Formatting
  ...
  def format_datetime_utc7_backend(val, options)
    return val unless val.kind_of?(Time) || val.kind_of?(Date)
    options[:tz] = "UTC"
  ...
  def format_int_to_utc7_time(val, _options = {})
    hours = val/60
    minutes = val%60
    "#{hours.to_s.rjust(2, '0')}:#{minutes.to_s.rjust(2, '0')} UTC+07:00"
  end

  def format_default_retention(val, _options = {})
    if val == -1
      'Default'
    else
      val
  ...
  def format_browser_info(val, _options = {})
    val = format_string_to_hash(val)
    if val['user_TZO'].to_i > 0
      tz = "UTC+#{val['user_TZO']}"
    elsif val['user_TZO'].to_i < 0
      tz = "UTC#{val['user_TZO']}"
    elsif val['user_TZO'].to_i == 0
      tz = ""
    else
    ...
    "Timezone: #{tz} | Browser: #{val['name_ui']} v#{val['version']} | OS: #{val['os_ui']}"
  end
  ...
  def format_string_to_hash(val)
    modified_string = val.gsub(/:(\w+)/){"\"#{$1}\""}
                        .gsub('=>', ':')
                        .gsub("nil", "null")
