...
class MiqScheduleWorker::Runner < MiqWorker::Runner
  ...
  def schedules_for_all_roles
    ...
    # Schedule - Check for password expiration
    scheduler.schedule_every(worker_settings[:check_expire_password_interval],
                             :tags =>[schedule_category]) { enqueue([:job_check_expire_password]) }
