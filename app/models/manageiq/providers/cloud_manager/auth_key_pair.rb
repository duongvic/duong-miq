class ManageIQ::Providers::CloudManager::AuthKeyPair < ::Authentication
  ...
  def self.create_key_pair_queue(userid, ext_management_system, options = {})
    ...
    queue_opts = {
      ...
      :args        => [ext_management_system.id, userid, options]
    }

    MiqTask.generic_action_with_callback(task_opts, queue_opts)
  end

  def self.create_key_pair(ems_id, userid, options)
    ...
    # CAS(khanhct) update tenant to database
  ...
  #
  def delete_key_pair_queue(userid, id)
    task_opts = {
