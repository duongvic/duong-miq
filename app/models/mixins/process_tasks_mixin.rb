  ...
  module ClassMethods
    ...
    def invoke_tasks_local(options)
      ...
      instances.zip(tasks) do |instance, task|
        ...
        msg = "#{ui_lookup(:model => instance.type)} [#{instance.name}] action '#{options[:task]}' initiated"
        msg = "Record [#{ui_lookup(:model => instance.type)}: '#{instance.name}'] destroyed" if options[:task] == 'destroy'
