...
module RetirementMixin
  ...
  def finish_retirement
    ...
    mark_retired
    message = "#{self.class.base_model.name}: [id:<#{id}>, name:<#{name}>] has been retired on: [#{retires_on.strftime("%x %R %Z")}]"
    ...
    if retired?
      delete
      release_netbox_ips
      delete_backup_service_vm
    end

  end

  def release_netbox_ips
    self.ipaddresses.each do |addr|
      ips = NetboxClientRuby.ipam.ip_addresses.filter(address: addr.to_s, tenant: User.current_user.userid)
      ips.each do |ip|
        ip.delete
      end
    end
  rescue Exception => e
    _log.error "Error releasing netbox IP: #{e.code} - #{e.details}"
  end

  def delete_backup_service_vm
    ...
    backup_vm = GrpcVm::VM.new(
      :name => self.name,
      :ems_ref => self.ems_ref,
      :user_id => self.evm_owner_id,
      )
    backup_stub.delete_vm(backup_vm)
  rescue GRPC::BadStatus => e
    _log.error "Error deleting backup service VM: #{e.code} - #{e.details}"
  end
  ...
  def raise_audit_event(event_name, message, requester = nil)
    event_hash = {
      ...
      :action       => AuditEvent.actions[:delete],
    }
