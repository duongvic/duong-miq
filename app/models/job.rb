class Job < ApplicationRecord
  ...
  def self.check_user_expire_password
    _now = Time.now.utc
    next_date = _now + 10.days
    # CAS(lamps) Refactored from last_password_change
    User.where('password_expiration between ? and ?', _now, next_date).each do |user|
      duration = (user.password_expiration - _now) / (60 * 60 * 24)
      user_info = {
        :email    => user.email,
      ...
      MiqAction.queue_user_email(:user_expire_password_mail, @mail_opts)
    end
  end
