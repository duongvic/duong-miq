class CloudVolume < ApplicationRecord
  ...
  def self.create_volume_queue(userid, ext_management_system, options = {})
    task_opts = {
      :action => "creating Volume for user #{userid}",
      :userid => userid
    }
  ...
  def update_status_volume_queue(userid, options = {})
    task_opts = {
      :action => "updating status Volume for user #{userid}",
      :userid => userid
    }

    queue_opts = {
      :class_name  => self.class.name,
      :method_name => 'update_status_volume',
      :instance_id => id,
      :role        => 'ems_operations',
      :queue_name  => ext_management_system.queue_name_for_ems_operations,
      :zone        => ext_management_system.my_zone,
  ...
  def update_status_volume(options = {})
    raw_update_status_volume(options)
  end

  def raw_update_status_volume(_options = {})
    raise NotImplementedError, _("raw_update_status_volume must be implemented in a subclass")
  end
  ...
  def update_volume_queue(userid, options = {})
    task_opts = {
      :action => "updating Volume for user #{userid}",
      :userid => userid
    }
  ...
  def delete_volume_queue(userid)
    task_opts = {
      :action => "deleting Volume for user #{userid}",
      :userid => userid
    }
