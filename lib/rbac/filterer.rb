module Rbac
  ...
  class Filterer
    ...
    def self_service_ownership_scope?(miq_group, klass)
      ...
      klass.respond_to?(:user_or_group_owned)
    ...
    def scope_targets(klass, scope, rbac_filters, user, miq_group)
      ...
      if apply_rbac_directly?(klass)
        # CAS(khanhct) Ignore restriction for klass TemplateCloud
        filtered_ids = klass == TemplateCloud ? nil : calc_filtered_ids(scope, rbac_filters, user, miq_group, nil)
        scope_by_ids(scope, filtered_ids)
